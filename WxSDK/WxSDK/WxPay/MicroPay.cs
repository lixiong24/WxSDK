...
namespace WxSDK.WxPay
{
	...
	public class MicroPay
	{
		...
		public static string Run(string body, string total_fee, string auth_code)
		{
			...
			WxData result = WxPayApi.Micropay(data, 10); //提交被扫支付，接收返回结果
			...
			if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
			{
				...
				throw new WxException("Micropay API interface call failure, return_msg : " + returnMsg);
			}
			...
			while (queryTimes-- > 0)
			{
				...
				WxData queryResult = Query(out_trade_no, out succResult);
				//如果需要继续查询，则等待2s后继续
				if (succResult == 2)
				{
					Thread.Sleep(2000);
			...
			if (!Cancel(out_trade_no))
			{
				...
				throw new WxException("Reverse order failure！");
			}
		...
		/// <returns></returns>
		public static WxData Query(string out_trade_no, out int succCode)
		{
			WxData queryOrderInput = new WxData();
			queryOrderInput.SetValue("out_trade_no", out_trade_no);
			WxData result = WxPayApi.OrderQuery(queryOrderInput);
		...
		/// <summary>
		/// 撤销订单，如果失败会重复调用10次，false表示撤销失败，true表示撤销成功
		/// </summary>
		/// <param name="out_trade_no">商户订单号</param>
		...
		public static bool Cancel(string out_trade_no, int depth = 0)
		{
			...
			WxData reverseInput = new WxData();
			reverseInput.SetValue("out_trade_no", out_trade_no);
			WxData result = WxPayApi.Reverse(reverseInput);
